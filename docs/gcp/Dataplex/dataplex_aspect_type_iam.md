## 🛡️ Policy Deployment Engine: `dataplex_aspect_type_iam`

This section provides a concise policy evaluation for the `dataplex_aspect_type_iam` resource in GCP.

Reference: [Terraform Registry – dataplex_aspect_type_iam](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/dataplex_aspect_type_iam)
...
| `aspect_type_id` |  | false | false | None | None | None |
...
| `role` | `google_dataplex_aspect_type_iam_binding` can be used per role. Note that custom roles must be of the format `[projects|organizations]/{parent-name}/roles/{role-name}`. | false | false | None | None | None |
