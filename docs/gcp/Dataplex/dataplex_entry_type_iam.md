## 🛡️ Policy Deployment Engine: `dataplex_entry_type_iam`

This section provides a concise policy evaluation for the `dataplex_entry_type_iam` resource in GCP.

Reference: [Terraform Registry – dataplex_entry_type_iam](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/dataplex_entry_type_iam)
...
| `entry_type_id` |  | false | false | None | None | None |
...
| `role` | `google_dataplex_entry_type_iam_binding` can be used per role. Note that custom roles must be of the format `[projects|organizations]/{parent-name}/roles/{role-name}`. | false | false | None | None | None |
