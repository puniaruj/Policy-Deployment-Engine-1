## 🛡️ Policy Deployment Engine: `dataplex_lake_iam`

This section provides a concise policy evaluation for the `dataplex_lake_iam` resource in GCP.

Reference: [Terraform Registry – dataplex_lake_iam](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/dataplex_lake_iam)
...
| `role` | `google_dataplex_lake_iam_binding` can be used per role. Note that custom roles must be of the format `[projects|organizations]/{parent-name}/roles/{role-name}`. | false | false | None | None | None |
