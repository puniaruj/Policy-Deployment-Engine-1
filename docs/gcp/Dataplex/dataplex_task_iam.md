## 🛡️ Policy Deployment Engine: `dataplex_task_iam`

This section provides a concise policy evaluation for the `dataplex_task_iam` resource in GCP.

Reference: [Terraform Registry – dataplex_task_iam](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/dataplex_task_iam)
...
| `lake` | Used to find the parent resource to bind the IAM policy to | false | false | None | None | None |
| `task_id` |  | false | false | None | None | None |
...
| `role` | `google_dataplex_task_iam_binding` can be used per role. Note that custom roles must be of the format `[projects|organizations]/{parent-name}/roles/{role-name}`. | false | false | None | None | None |
