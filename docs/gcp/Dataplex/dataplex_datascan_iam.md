## 🛡️ Policy Deployment Engine: `dataplex_datascan_iam`

This section provides a concise policy evaluation for the `dataplex_datascan_iam` resource in GCP.

Reference: [Terraform Registry – dataplex_datascan_iam](https://registry.terraform.io/providers/hashicorp/google/latest/docs/resources/dataplex_datascan_iam)
...
| `data_scan_id` |  | false | false | None | None | None |
...
| `role` | `google_dataplex_datascan_iam_binding` can be used per role. Note that custom roles must be of the format `[projects|organizations]/{parent-name}/roles/{role-name}`. | false | false | None | None | None |
